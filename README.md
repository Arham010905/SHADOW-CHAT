...
preventing long-term storage or recovery. 

Key Features:
1.End-to-End Encryption — Messages are encrypted on the sender's device and only decrypted on the recipient's,
2. keeping your chats private from intermediaries, servers, or third parties.
3. Auto-Delete Functionality — Ephemeral messaging system where chats self-destruct automatically,
ideal for sensitive discussions. 
3.Simple Chat Box Interface — A clean, user-friendly chat box for real-time communication without unnecessary complexity.
4.Privacy by Design — No persistent logs,
